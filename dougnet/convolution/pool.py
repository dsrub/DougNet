...
def MP2d(Z, H_K, W_K, stride=1, return_inds=False):
    """
    ...
    given by looping over rows, columns.  This seems to mimic the pytorch behavior in case 
    of a tie.  
    """
...
def _dZ_MP2d(dZ_MP, Z, I_max, J_max, H_K, W_K, stride=1):
    """
    Given the gradient of the loss wrt the output of the max pool 
    operation, return dL/dZ, the gradient of the loss wrt the input
    tensor.
    """
...
def GAP2d(Z):
    ...
    _, _, H, W = Z.shape
    Z_GAP = (np.einsum('bhij->bh', Z) / (H * W)).astype(Z.dtype)
    return Z_GAP

def _dZ_GAP2d(dZ_GAP, Z):
    """
    Given the gradient of the loss wrt the output of the global average 
    pool operation, return dL/dZ, the gradient of the loss wrt the input
    tensor.
    """
    # get shapes
    N, C, H, W = Z.shape
    ...
    dZ = np.zeros((N, C, H, W), dtype=Z.dtype) + (dZ_GAP.reshape(N, C, 1, 1) / (H * W)).astype(Z.dtype)
    return dZ
