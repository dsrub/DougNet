...
def test_BN1d_1epoch(eps):
    ...
    Z_BN = dn.BN1d(Z1.node, 100, eps=eps)
    A1 = dn.Tanh(Z_BN)
    
    Yhat = dn.Linear(A1, 10, 100)
    L = dn.SoftmaxCrossEntropyLoss(Yhat.node, Y)
    ...
    assert np.allclose(model_pytorch[1].weight.data.numpy(), Z_BN.gamma.output.reshape(-1), rtol=1e-6, atol=1e-06)
    assert np.allclose(model_pytorch[1].bias.data.numpy(), Z_BN.beta.output.reshape(-1), rtol=1e-6, atol=1e-06)

    # check running stats
    assert np.allclose(model_pytorch[1].running_mean.numpy(), Z_BN.running_mean.reshape(-1), rtol=1e-5, atol=1e-05)
    assert np.allclose(model_pytorch[1].running_var.numpy(), Z_BN.running_var.reshape(-1), rtol=1e-5, atol=1e-05)
...
@pytest.mark.parametrize("eps", [1e-5, 1])
def test_BN1d_multiple_epochs(eps):
    ...
    progress = ProgressHelper(EPOCHS, X, Y, Yhat.node, L, progress_metric=accuracy, verbose=False)
    for epoch in range(EPOCHS):
        ...
        progress.update(epoch, X_train, Y_train, X_val, Y_val)
